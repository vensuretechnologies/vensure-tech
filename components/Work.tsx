...
const projects = [
  {
    ...
    gradient: 'from-[#1a0a3a] via-[#2d1b69] to-[#a855f7]',
    size: 'featured',
  },
  {
...
const CardDecoration = ({ index }: { index: number }) => {
  if (index === 0) return (
    <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 400 500" fill="none">
      <circle cx="320" cy="120" r="180" stroke="rgba(168,85,247,0.4)" strokeWidth="0.5" />
      ...
      <line x1="0" y1="250" x2="400" y2="250" stroke="rgba(168,85,247,0.15)" strokeWidth="0.5" />
      <line x1="200" y1="0" x2="200" y2="500" stroke="rgba(168,85,247,0.15)" strokeWidth="0.5" />
    </svg>
  )
  if (index === 1) return (
    <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 400 300" fill="none">
      <rect x="80" y="60" width="240" height="180" stroke="rgba(168,85,247,0.3)" strokeWidth="0.5" />
      <rect x="120" y="90" width="160" height="120" stroke="rgba(192,132,252,0.4)" strokeWidth="0.5" />
      <line x1="80" y1="60" x2="320" y2="240" stroke="rgba(168,85,247,0.2)" strokeWidth="0.5" />
      <line x1="320" y1="60" x2="80" y2="240" stroke="rgba(168,85,247,0.2)" strokeWidth="0.5" />
    </svg>
  )
  return (
    <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 400 300" fill="none">
      <polygon points="200,30 370,270 30,270" stroke="rgba(168,85,247,0.3)" strokeWidth="0.5" fill="none" />
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="reveal flex items-end justify-between mb-16">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-mono text-xs tracking-[0.25em] text-purple-bright/70 uppercase">Selected Work</span>
            </div>
          ...
          <a
            href="#contact"
            className="hidden md:flex items-center gap-2 font-body text-xs font-semibold tracking-widest uppercase text-purple-bright/50 hover:text-purple-bright transition-colors group"
          >
            View All Cases
        ...
        {/* Work grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-purple-glow/8">
          {/* Featured */}
          <div className="reveal md:row-span-2 group relative overflow-hidden bg-surface min-h-[480px] cursor-pointer">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-10">
              <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-purple-bright mb-3">{projects[0].tag}</span>
            ...
            <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-1 group-hover:translate-y-0">
              <span className="font-mono text-xs text-purple-bright border border-purple-glow/30 px-3 py-1">View Case →</span>
            </div>
          </div>
          ...
          {[projects[1], projects[2]].map((p, i) => (
            <div key={i} className="reveal group relative overflow-hidden bg-surface min-h-[220px] cursor-pointer">
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-8">
                <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-purple-bright mb-2">{p.tag}</span>
              ...
              <div className="absolute top-5 right-5 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <span className="text-purple-glow text-lg">↗</span>
              </div>
            </div>
          ))}
        </div>
